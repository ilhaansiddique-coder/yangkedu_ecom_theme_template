...
const RAW: [string, string, string][] = [
  ...
  ["Grenada", "GD", "+1"], ["Guatemala", "GT", "+502"], ["Guinea", "GN", "+224"],
  ...
  ["Uzbekistan", "UZ", "+998"], ["Vanuatu", "VU", "+678"], ["Vatican City", "VA", "+39"],
...
export const findCountry = (code: string): Country =>
  COUNTRIES.find((c) => c.code === code) ?? COUNTRIES.find((c) => c.code === "IN")!;

/** Filter by country name or dial code (autocomplete). */
export function searchCountries(query: string): Country[] {
  const q = query.trim().toLowerCase();
  if (!q) return COUNTRIES;
  const digits = q.replace(/[^\d+]/g, "");
  return COUNTRIES.filter(
    (c) => c.name.toLowerCase().includes(q) || (digits && c.dial.includes(digits)) || c.code.toLowerCase() === q,
  );
}
